...
* For the regular Anbox Cloud deployments, streaming can now be enabled when launching an instance using the `--enable-streaming` option. Existing installations will require the charm deployment for Anbox Cloud to be manually upgraded to use the new relation between the AMS and the stream agent.
...
* Creating instances with streaming enabled using the `enable-streaming` option is not available for the Anbox Cloud Appliance.
