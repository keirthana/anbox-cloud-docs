...
* [LP 2113771](https://bugs.launchpad.net/anbox-cloud/+bug/2113771) When upgrade charms from 1.26.0 to 1.26.1, the AMS node controller ends up in an error state. There is no action required here because the AMS node controller is deprecated since 1.26.0 and the port forwarding is instead done by LXD.
